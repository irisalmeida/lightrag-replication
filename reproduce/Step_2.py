...
def get_summary(context, tot_tokens=400, max_char_window=3000):
    if len(context) > max_char_window:
        half_window = max_char_window // 2
        context = f"{context[:half_window]}\n...\n{context[-half_window:]}"

    tokens = tokenizer.tokenize(context)

    if len(tokens) <= tot_tokens:
        return tokenizer.convert_tokens_to_string(tokens)

    half_tokens = max(1, tot_tokens // 2)
...
clses = [os.environ.get("LIGHTRAG_CLS", "agriculture")]  # controlado por run_all_domains.py
for cls in clses:
    #with open(f"../datasets/unique_contexts/{cls}_unique_contexts.json", mode="r") as f:
    with open(f"./datasets/UltraDomain_2samples_unique_contexts/{cls}_unique_contexts.json", mode="r") as f:
